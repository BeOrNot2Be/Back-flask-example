...
class LikesUser(Resource):
    def get(self, id):
        ...
        query = database.prepare("select count(*) from user_liked where like_user_id = $1")
    ...
    def post(self, id):
        ...
        query = database.prepare("select exists(select true from user_liked where user_id = $1 and like_user_id = $2)")
        liked = query(int(user_id), int(id))[0][0]

        if liked:
            query = database.prepare("DELETE FROM user_liked WHERE user_id = $1 and like_user_id = $2")
        ...
        if not liked:
            query = database.prepare("INSERT INTO user_liked (user_id, like_user_id) VALUES ($1, $2)")
